...
do
  ...
  for hosts in `cat nodes.1`; do ssh -i /home/ubuntu/vcider.pem ubuntu@$hosts "sudo /etc/init.d/cassandra stop  >/dev/null"; done
  echo "I: cleaning up  cassandra files.."
  for hosts in `cat nodes.1`; do ssh -i /home/ubuntu/vcider.pem ubuntu@$hosts "sudo rm -rf /mnt/cassandra/*"; done

  echo "I: updating puppet configs.."
  for hosts in `cat nodes.1`; do ssh -i /home/ubuntu/vcider.pem ubuntu@$hosts "sudo puppetd --test >/dev/null"; done
  sleep 5
  for hosts in `cat nodes.1`; do ssh -i /home/ubuntu/vcider.pem ubuntu@$hosts "sudo puppetd --test >/dev/null"; done

  sleep 2
  echo "I: starting cassandra.."
  for hosts in `cat nodes.1`; do  ssh -i /home/ubuntu/vcider.pem ubuntu@$hosts "sudo /etc/init.d/cassandra start >/dev/null 2>&1"; done

  #echo "I: sleeping for one minute.."
  sleep 5
  #echo "I: Ring Status before Test Run:"
  #ssh -i /home/ubuntu/vcider.pem ubuntu@`head -n1 nodes` "sudo /opt/cassandra/bin/nodetool -h localhost ring" 
  ...
  echo "I: Ring Status" 
  ssh -i /home/ubuntu/vcider.pem ubuntu@`head -n1 nodes.1` "sudo /opt/cassandra/bin/nodetool -h localhost ring"
  ...
  ssh -i /home/ubuntu/vcider-cal.pem ubuntu@50.18.181.61 "sudo cp /etc/puppet/manifests/nodes.pp.4 /etc/puppet/manifests/nodes.pp"

  echo "I: stopping cassandra.."
  for hosts in `cat nodes.1`; do ssh -i /home/ubuntu/vcider.pem ubuntu@$hosts "sudo /etc/init.d/cassandra stop >/dev/null"; done
  echo "I: cleaning up  cassandra files.."
  for hosts in `cat nodes.1`; do ssh -i /home/ubuntu/vcider.pem ubuntu@$hosts "sudo rm -rf /mnt/cassandra/*"; done
  #for hosts in `cat nodes`; do ssh -i /home/ubuntu/vcider.pem ubuntu@$hosts "sudo /etc/init.d/cassandra start"; done                           
  echo "I: updating puppet configs.."
  for hosts in `cat nodes.1`; do ssh -i /home/ubuntu/vcider.pem ubuntu@$hosts "sudo puppetd --test >/dev/null"; done
  sleep 5
  for hosts in `cat nodes.1`; do ssh -i /home/ubuntu/vcider.pem ubuntu@$hosts "sudo puppetd --test >/dev/null"; done
  ...
  #echo "Options: $STRESS_OPTIONS"
  ./get_openvpn_ips.sh
  ./stress -D nodes.ovpn $STRESS_OPTIONS | tee tmp
